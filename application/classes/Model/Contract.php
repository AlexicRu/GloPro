...
class Model_Contract extends Model
{
	...
	public static function editContract($contractId, $params)
	{
		if(
			...
			empty($params['contract']['DATE_BEGIN'])
		){
			return false;
		}

        if(!empty($params['contract']['DATE_END']) && Date::dateDifference($params['contract']['DATE_BEGIN'], $params['contract']['DATE_END']) > 0) {
		...
		$data = [
            ...
            'p_date_end' 		=> !empty($params['contract']['DATE_END']) ? Date::format($params['contract']['DATE_END']) : Date::DATE_MAX,
        ...
        $data1 = [
            ...
            'p_autoblock_flag_date' 	=> $params['settings']['scheme'] == self::PAYMENT_SCHEME_LIMIT ? Date::format($params['settings']['AUTOBLOCK_FLAG_DATE']) : Date::DATE_MAX,
