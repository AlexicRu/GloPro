...
<script>
    ...
    function renderAjaxPaginationClientBillsList(data, block) {
        for(var i = 0 in data){
            ...
            tpl.append('<a href="/reports/generate?type=<?=Model_Report::REPORT_TYPE_BILL?>&format=pdf&contract_id=' + data[i].CONTRACT_ID + '&invoice_number=' + data[i].INVOICE_NUMBER + '" class="btn waves-effect waves-light btn_small" target="_blank"><i class="icon-download"></i> Скачать</a>');

            <?if(Access::allow('download_bill_as_xls')){?>
            tpl.append('&nbsp;<a href="/reports/generate?type=<?=Model_Report::REPORT_TYPE_BILL?>&format=xls&contract_id=' + data[i].CONTRACT_ID + '&invoice_number=' + data[i].INVOICE_NUMBER + '" class="btn waves-effect waves-light btn_small" target="_blank">В Excel</a>');
            <?}?>
